<#
.SYNOPSIS
    Patch provisioned K3s worker nodes via Tailscale SSH.
.DESCRIPTION
    Fixes: registries.yaml, expand-rootfs, NTP, root password lock.
    Run from any Tailscale-connected Windows machine.
.EXAMPLE
    .\patch-node.ps1 node-a5dd21
    .\patch-node.ps1 node-a5dd21 node-b3cf42
#>
...
# Write payload to temp file with LF line endings (no BOM).
# Piping a PS string sends CRLF — bash sees \r and chokes.
$TempFile = Join-Path ([System.IO.Path]::GetTempPath()) "patch-node-$PID.sh"
[System.IO.File]::WriteAllText($TempFile, $PatchScript.Replace("`r`n", "`n"), [System.Text.UTF8Encoding]::new($false))

try {
    foreach ($Node in $Nodes) {
        Write-Host ""
        Write-Host "=== Patching $Node ===" -ForegroundColor Cyan
        Write-Host ""

        & cmd /c "tailscale ssh `"root@$Node`" -- bash -s < `"$TempFile`""
        if ($LASTEXITCODE -ne 0) {
            Write-Host "=== $Node FAILED (exit $LASTEXITCODE) ===" -ForegroundColor Red
            $Failed += $Node
        } else {
            Write-Host "=== $Node patched ===" -ForegroundColor Green
        }
    }
} finally {
...
Write-Host "Verify: tailscale ssh root@k3s-control-0 -- k3s kubectl get nodes"
